...
export default function AddListingPage() {
  ...
  const handleCreateBook = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Kitap oluşturulurken hata oluştu. Bilgileri kontrol et.");
    } finally {
      setLoading(false);
    }
  };
        ...
        {step === 1 && (
          <form onSubmit={handleCreateBook} className="space-y-6 animate-fade-in">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className={labelClass}>ISBN <span className="text-theme-muted/50 font-normal">(Opsiyonel)</span></label>
                <input type="text" className={inputClass} placeholder="Örn: 978..." value={bookData.isbn} onChange={e => setBookData({...bookData, isbn: e.target.value})} />
              </div>
              <div>
                <label className={labelClass}>Basım Yılı</label>
                <input type="number" required className={inputClass} placeholder="Örn: 2020" value={bookData.published_year} onChange={e => setBookData({...bookData, published_year: e.target.value})} />
              </div>
              <div>
                <label className={labelClass}>Sayfa Sayısı</label>
                <input type="number" required className={inputClass} placeholder="Örn: 687" value={bookData.page_count} onChange={e => setBookData({...bookData, page_count: e.target.value})} />
              </div>
              <div className="md:col-span-2">
